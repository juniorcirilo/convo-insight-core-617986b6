...
export const AddInstanceDialog = ({ open, onOpenChange }: AddInstanceDialogProps) => {
  ...
  const webhookUrl = `${API_URL.replace('/api', '')}/api/whatsapp/webhooks/evolution`;
  ...
  const handleApplyToEvolution = async (force = false, instanceId?: string) => {
    ...
    if (instanceId) {
      ...
      try {
        const payload = {
          instanceId,
          webhookUrl: `${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:3001'}/api/whatsapp/webhooks/evolution`,
